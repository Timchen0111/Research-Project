...
switch mode
     ...
     case 'broadcast'
        disp('BROADCAST')
        prompt = "Please enter the acceptable SINR value to enter broadcast: ";
        value = input(prompt);
        multi = false;
     otherwise
        error( 'WRONG USE.' )
...
for i=1:UE_num
    ...
    gNB = call_gNB(gNB,UE(i).now_gNB,UE(i),noise,UE,bw,1,value,multi);
...
for t=1:time %600 %1 minutes
    ...
    for i=1:UE_num
        ...
        if UE(i).now_gNB~=now
            ...
            gNB = call_gNB(gNB,old,UE(i),noise,UE,bw,2,value,multi);%Remove UE
            UE(i).unicast = false;
            gNB = call_gNB(gNB,now,UE(i),noise,UE,bw,1,value,multi);%add UE
            if gNB(UE(i).now_gNB).saveMBS(1)==true
                UE(i).MBS = gNB(UE(i).now_gNB).saveMBS(2);
                gNB(UE(i).now_gNB).saveMBS = [false,0];
                %disp(UE(i))
            ...
            if gNB(now).unicast_member(i) == true 
                UE(i).SINR = SINR(UE(i),UE(i).now_gNB,gNB,noise);
                gNB = call_gNB(gNB,UE(i).now_gNB,UE(i),noise,UE,bw,5,value,multi);
