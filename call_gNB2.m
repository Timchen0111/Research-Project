function g = call_gNB2(gNB,tgNB_num,UE,noise,allUE,bw,command,value)
    g = gNB;
    %disp(g(tgNB_num))
    if command == 1 %add UE 
            %disp('ADD UE')
            ...
            if UE.SINR>value
                multi = true;
                g = call_gNB2(g,tgNB_num,UE,noise,allUE,bw,3,value);%Add MBS
    ...
    if command == 2
            ...
            if UE.MBS ~= -1
                g = call_gNB2(gNB,tgNB_num,UE,noise,allUE,bw,4,value);%Remove MBS
            end
    end
    if command == 3
            %disp('ADD MBS')
    ...
    if command == 5
            ...
            if UE.SINR>value
                ...
                g = call_gNB2(g,tgNB_num,UE,noise,allUE,bw,3,value);
                %disp('LEAVE UNICAST')
                UE.num
            else
                g(tgNB_num).unicast_member(UE.num) = true;
